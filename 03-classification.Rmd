...
We can also make an ROC curve for every fold  (see [Google Developers](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc) for more details).
...
And this is our final model result. Remember that if a model fit to the training dataset also fits the test dataset well, minimal *overfitting* has taken place. This seems to be also the case in our example. 
...
last_fit_rf %>% 
  collect_predictions() %>% 
  roc_curve(price_class, .pred_class) %>% 
  autoplot()
