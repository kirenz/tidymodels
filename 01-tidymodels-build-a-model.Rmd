...
This next step is not necessary but if we would like to check if all of the preprocessing steps above actually work, we can proceed as follows to take a look at the data:

```{r}
# only if we would like to take a look at 
# the result of all of the data preprocessing 

# prep() performs the recipe on training data
prepped_recipe <- prep(housing_rec) 

# juice() extracts the data from the 
juice(prepped_recipe) # show preprocessed data
...
To assess the model predictions, we plot the predictions on the y-axis and the real median house value at the x-axis. This means if our model would have made no mistakes, all points would lie on the red diagonal line. 
