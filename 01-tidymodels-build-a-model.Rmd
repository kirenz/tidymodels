...
The function `ggscatmat` from the package `GGally` creates a matrix with scatterplots, densities and correlations for numeric columns. In our code, we enter the dataset `housing_df`, choose columns 8 to 12, a color column for our categorical variable `ocean_proximity`, and an alpha level of 0.8 (for transparency).
...
We could also use the function `cor.test()` to calculate the statistical significance of Spearman's rank correlation between the variables:
...
Consequently we will only use this four variables as well as `ocean_proximity` as predictors in our model.
...
Next, we’ll preprocess our data before training models. Therefore, we mainly use the tidymodels packages `recipes`. `Recipes` are built as a series of optional data preparation steps, such as:
...
First we insert our new variables in the data and drop the variables we will note use in the model. We keep `longitude` and `latitude` to be able to map the data in a later stage. Furthermore, we make a new split. 
...
housing_df_new <-
  housing_df %>% 
   mutate(
     ...
     bedrooms_per_room = total_bedrooms/total_rooms,
     population_per_household = population/households) %>% 
  select(-total_rooms, -total_bedrooms, -housing_median_age,
...
Let’s create a `recipe` for our regression models (Lasso, Random Forest, XGBoost):
...
* `step_novel()` converts all nominal variables to factors and take cares of other issues related to categorical variables.
...
We want to assess three different models. The process is always as follows:
...
To use this model correctly, we need to add a data normalization step as well as a step to declare the number of freedoms in our model. We will update our recipe at a later step (when we create the workflows).
...
Note that we first need to normalize (z-standardize) the data for our Lasso model. We only need to perform this step for some models and therefore just add this step to a new recipe we call `housing_rec_lasso`: 

```{r}

# chnage the recipe for Lasso 
...
Note that we first need to normalize (z-standardize) the data for a natural spline model. Furthermore, we need to declare the degrees of freedom (in `step_ns()`).

```{r}

library(splines)

# change the recipe for natural spline 
housing_rec_spline <- # create a new recipe object
  housing_rec %>%  # use the old recipe
  step_normalize(all_numeric(), -all_outcomes()) %>% 
  step_ns(median_income, deg_free = 3) %>% 
  step_ns(bedrooms_per_room, deg_free = 3) 

# bundle recipe and model
lasso_wflow <- # new workflow object
 workflow() %>% # use workflow function
 add_recipe(housing_rec_spline) %>%   # use the Lasso recipe
 add_model(spline_spec)   # add your model spec
...
Now it's time to fit the best model (in our case the XGBoost model) one last time to the full training set and evaluate the resulting final model on the test set. Tidymodels provides the function [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.html) which does all of the described steps for us. We just need to provide the workflow object of the best model as well as the data split object (not the training data).
